...
// Use multiple times
const html1 = highlighter.codeToHtml(code1, { lang: 'ts', theme: 'github-dark' })
const html2 = highlighter.codeToHtml(code2, { lang: 'tsx', theme: 'github-light' })
...
export async function Code({ code, lang }: { code: string; lang: string }) {
  ...
  return (
    <div
      className="[&_pre]:p-4 [&_pre]:rounded-lg"
...
const myTheme = JSON.parse(
  await fs.readFile('./my-theme.json', 'utf-8')
)
