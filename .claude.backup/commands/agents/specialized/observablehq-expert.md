...
// Scatter plot
Plot.dot(data, {x: "gdp", y: "lifeExpectancy", r: "population", fill: "continent"}).plot()
...
Plot.plot({
  ...
  marks: [
    Plot.frame(),
    Plot.dot(data, {x: "price", y: "quantity", fill: "type"})
  ]
})
```
...
Plot.barY(data, Plot.normalizeY(
  Plot.stackY({x: "year", y: "value", fill: "category"})
)).plot()
...
```javascript
// Fetch JSON
earthquakes = fetch("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson")
  .then(r => r.json())
