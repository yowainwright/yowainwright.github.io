...
async function searchOrders(query: string) {
  const result = await elasticsearch.search({
    ...
    body: {
      query: {
        multi_match: { query, fields: ["userName", "items.productName"] },
      },
    },
  });
...
app.get("/users", async (req, res) => {
  ...
  const users = await prisma.user.findMany({
    select: requestedFields ? Object.fromEntries(requestedFields.map((f) => [f, true])) : undefined, // All fields if not specified
  });
