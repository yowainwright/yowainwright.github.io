---
title: "Why Pastoralist: Nobody Remembers Why Their Overrides Exist And Now You Don't Need To"
date: "2026-02-20"
path: "/why-pastoralist"
meta: "This post covers why Pastoralist, a tool that tracks, scans, and cleans npm dependency overrides, was built. To hopefully make this clearer to you,  we looked at 10 major OSS projects, found 60+ untracked overrides across them, and create pull requests with pastoralist to fix the issue"
categories:
  - code
  - security
  - dependencies
...
|---|---|---|---|---|
| **n8n** | ~51k | `pnpm.overrides` | 58 | ~17 (axios, ws, semver, gaxios, multer, pug, nodemailer, form-data, tar-fs, brace-expansion, bn.js, jws) |
| **Storybook** | ~85k | `resolutions` | 19 | ~3 (@babel/runtime, @babel/traverse, @babel/types) |
...
| **Cal.com** | ~34k | `resolutions` | 41 | 3+ (brace-expansion, lodash, tar) |

**That's 135+ overrides across 10 projects. None of them document why.**

---

Some observations:

- **n8n** has 58 overrides. Some are security patches (ws, semver, axios). Some are compatibility pins (date-fns, chokidar, esbuild). Some are forks (`expr-eval@2.0.2` remapped to `npm:expr-eval-fork@3.0.0`). None of this is documented in the `package.json` itself.
...
- **Supabase** had 4 overrides when this was first written and has since removed all of them--the right outcome, but only possible if you know they're there and why.
...
We ran pastoralist against these same projects to find out how many of those overrides are still doing anything:

| Project | Overrides | Unused |
|---------|-----------|--------|
| n8n | 58 | 56 |
| TanStack Router | 51 | 41 |
| Cal.com | 41 | 35 |
| Next.js | 16 | 3 |
| Prisma | 5 | 5 |

**140 overrides across 5 projects are no longer doing anything.** Prisma is the clearest case: all 5 of their overrides are security patches for packages that have since left their dependency tree. The patches are carrying dead weight and the teams have no way to know without running a tool like pastoralist.
...
We timed pastoralist against the same projects above, including a full OSV security scan (`--security --provider osv`). No API key, no auth:
...
Under 500ms for the largest override sets, network call included. Next.js takes ~1s because it also evaluates patch files alongside its overrides. Projects with no overrides clock in under 300ms.

The OSV scan adds almost no overhead — n8n with 58 overrides ran faster with `--security` than without it.
